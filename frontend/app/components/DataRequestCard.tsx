      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {requestedTypes.map((dataType) => (
            <div
              key={dataType}
              className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 border border-slate-700/50 hover:border-blue-500/30 transition-colors"
            >
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="block w-full text-sm text-slate-300 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-cyan-500 file:text-white hover:file:bg-cyan-600 file:cursor-pointer file:transition-colors"
        ...
        {files.length > 0 && (
          <div className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
            <p className="text-xs font-medium text-slate-400 mb-2">Selected files ({files.length}):</p>
            <ul className="space-y-1.5">
              {files.map((file, index) => (
                <li key={index} className="flex items-center justify-between text-xs text-slate-300">
